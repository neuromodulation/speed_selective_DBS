# Plot fast vs slow for figure publication
...
f, axes = plt.subplots(1, 4, gridspec_kw={'width_ratios': [1, 1, 1, 1]}, figsize=(2.5, 2))
...
for block in range(2):
    ...
    ax.fill_between(x, y[0, :] - std[0, :], y[0, :] + std[0, :], color=colors_op[0], alpha=0.5)
    ax.fill_between(x, y[1, :] - std[1, :], y[1, :] + std[1, :], color=colors_op[1], alpha=0.5)
    # Adjust plot
...
for block in range(2):
    ...
    for j in range(2):
        bp = ax.boxplot(x=feature_av[:, j],
                    ...
                    flierprops=dict(marker='.', markerfacecolor=colors_op[j], markersize=0.5, markeredgecolor='none')
                    )
        bps.append(bp)  # Save boxplot for creating the legend

    # Add the individual lines
    for dat in feature_av:
        ax.plot(bar_pos[0], dat[0], marker='.', markersize=0.3, color=colors[0])
        ax.plot(bar_pos[1], dat[1], marker='.', markersize=0.3, color=colors[1])
