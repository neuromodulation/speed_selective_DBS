...
for i, target_name in enumerate(targets):
    ...
    for j in range(len(y)):
        ...
        x = conn_mat[idx_train, np.newaxis]
        y_train = y[idx_train]
    ...
    # Test for 100 shuffles
    """res_shuffle = np.zeros(800)
    for s in range(800):
        y_shuffle = y.copy()
    ...
    # Analyze the shuffled R and the true R
    percentile = np.sum(res_shuffle <= res[i])/len(res_shuffle)"""
    """plt.figure()
    plt.hist(res_shuffle, bins=100)
    plt.axvline(res[i], color="red")
    plt.title(f"p = {percentile} for {target_name}")"""
