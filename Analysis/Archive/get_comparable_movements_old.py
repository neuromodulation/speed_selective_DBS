...
for j in range(2):
    ...
    for k in range(stim.shape[0]):
        ...
        if j == 0:
            ...
            stim_idx_other = idx_all[idx_all > 96]
        else:
            stim_idx = idx_all[idx_all > 96]
            stim_idx_other = idx_all[idx_all < 96]
        ...
        for l, idx in enumerate(stim_idx):
            ...
            for x in tmp_idx:
                # Check that movement was not stimulated before and that the movement was not already selected
                if x not in stim_idx_other and x not in stim_idx and feature_tmp[x] not in feature_similar:
                    feature_similar[l] = feature_tmp[x]

                    # Add 1 at the position of the selected movement
                    if n == 0:
                        similar_speed[np.where(feature == feature_tmp[x])] = 1
                        break
                    # Save the movement n positions after the stimulation
                    elif n == 1 and x+1 not in [96*2, 96*4]:
                        similar_speed[np.where(feature == feature_tmp[x+1])] = 1
                        break
                    elif n == 2 and x+2 not in [96*2, 96*2+1, 96*4, 96*4+1]:
                        similar_speed[np.where(feature == feature_tmp[x+2])] = 1
                        break
                    elif n == 3 and x+3 not in [96*2, 96*2+1, 96*2+2, 96*4, 96*4+1, 96*4+2]:
                        similar_speed[np.where(feature == feature_tmp[x+3])] = 1
                        break
        ...
        plt.ylim([0, 9000])
        plt.title(med)
        plt.ylabel(f"Peak speed of stimulated \n and comparable movement", fontsize=11)

    # Save the similar speed matrix
    np.save(f"../../../Data/{med}/processed_data/{cond_names[j]}_{n}_similar_old.npy",
            similar_speed)
