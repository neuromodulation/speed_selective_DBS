...
for i in range(1, 3):
    ...
    feature_matrix_mean = np.nanmedian(feature_matrix[:, :, int(91*(i-1)):int(91*i)], axis=2)
    #feature_matrix_mean = np.nanmean(feature_matrix[:, :, 135:180], axis=2)
    ...
    # Add statistics
    z, p = scipy.stats.wilcoxon(feature_matrix_mean[:, 0], feature_matrix_mean[:, 1])
    print(p)
    res = scipy.stats.permutation_test(data=(feature_matrix_mean[:, 0], feature_matrix_mean[:, 1]), statistic=statistic,
