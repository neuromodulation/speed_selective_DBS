...
# Cut into epochs
epochs = mne.Epochs(raw, events=events, event_id=event_id, tmin=-0.35, tmax=0, baseline=None, reject_by_annotation=False)
...
for i, band in enumerate(bands):
    ...
    try:
        ...
        plt.subplot(2, 1, i + 1)
        sb.regplot(x=power_all[not_nan], y=feature_all[not_nan], label=label, scatter_kws={"color": "dimgrey", "alpha": 0.2}, line_kws={"color": "indianred"})
    ...
    # Adjust plot
    plt.subplots_adjust(bottom=0.15, left=0.1, top=0.9, wspace=0.3, hspace=0.3)
    plt.title(f"{band[0]}-{band[-1]} Hz Power", fontsize=13)

    # Save
    plot_name = os.path.basename(__file__).split(".")[0]
    dir_name = os.path.dirname(os.path.realpath(__file__)).split("\\")[-1]
    plt.savefig(f"../../../Figures/{dir_name}/{plot_name}_{sub}_{feature_name}.svg",
                format="svg", bbox_inches="tight", transparent=True)
    plt.savefig(f"../../../Figures/{dir_name}/{plot_name}_{sub}_{feature_name}.png",
