...
# Compute tfr
tfr = epochs.compute_tfr(method="morlet", freqs=frequencies, picks=target_chan_name, average=False, n_cycles=5)#frequencies/2)
# Smooth tfr
...
tfr1 = tfr[stim_idx].apply_baseline(baseline=baseline, mode=mode)
tfr2 = tfr[no_stim_idx].apply_baseline(baseline=baseline, mode=mode)
tfr1_av = tfr1.average()
tfr2_av = tfr2.average()
tfr_diff = tfr1_av.copy()
tfr_diff.data = tfr2_av.data #- tfr2_av.data #* 100

# Select data from channel of interest
power = tfr_diff.get_data(target_chan_name, tmin=tmin, tmax=tmax).squeeze()

# Plot
fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(1.5, 1.8))
im = ax.imshow(power, aspect="auto", origin="lower", cmap=cmap, extent=(tmin, tmax, np.min(frequencies), np.max(frequencies)), vmin=-2, vmax=2)
# Add colorbar
...
"""n_epochs, _, n_freqs, _ = tfr1._data.shape
times = tfr.times[(tfr.times >= tmin) & (tfr.times < tmax)]
res = np.zeros((2, n_freqs, len(times)))
tfr1_data = tfr1.get_data(target_chan_name, tmin=tmin, tmax=tmax).squeeze()
tfr2_data = tfr2.get_data(target_chan_name, tmin=tmin, tmax=tmax).squeeze()
for j, freq in enumerate(frequencies):
    print(f"Freq: {freq}")
    for k, time in enumerate(times):
        t, p = ttest_ind(a=tfr1_data[:, j, k], b=tfr2_data[:, j, k])
        res[0, j, k] = t
        res[1, j, k] = p
...
alphas = p.copy()
p[res[1, :, :] >= 0.05] = 1
alphas[res[1, :, :] < 0.05] = 0.1
alphas[res[1, :, :] >= 0.05] = 0.5
ax.imshow(p, aspect="auto", origin="lower", cmap="binary", extent=(tmin, tmax, np.min(frequencies), np.max(frequencies)), alpha=alphas)
"""
# Save figure
plot_name = os.path.basename(__file__).split(".")[0]
dir_name = os.path.dirname(os.path.realpath(__file__)).split("\\")[-1]
plt.savefig(f"../../../Figures/{dir_name}/{plot_name}_{target_chan_name}.pdf",
