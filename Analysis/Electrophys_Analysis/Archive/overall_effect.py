# PLot the power of all trials for both conditions and compare the average power values (for all channels)
...
n_norm = 5
n_cutoff = 5
channels = ["contra_LFP", "ipsi_LFP", "contra_ECOG", "ipsi_ECOG"]

# Prepare plotting
colors = ["#00863b", "#3b0086"]
colors_op = ["#b2dac4", "#b099ce"]
labels = ["Slow", "Fast"]
fig, axs = plt.subplots(2, len(channels), figsize=(14, 8))

# Loop over channels and create one for each
for i, channel in enumerate(channels):

    # Load the power values
    feature = np.load(f"../../../Data/{med}/processed_data/power_{band[0]}_{band[-1]}_{channel}.npy")
    n_datasets, _, _, n_trials = feature.shape

    # Detect and fill outliers (e.g. when subject did not touch the screen)
    np.apply_along_axis(lambda m: u.fill_outliers_nan_ephys(m), axis=3, arr=feature)
    #np.apply_along_axis(lambda m: u.fill_outliers_nan_ephys(m), axis=3, arr=feature)
    ...
    y = np.nanmedian(feature_smooth, axis=0)
    ...
    if n_norm!= 0: ax1.axhline(0, linewidth=1, color="black", linestyle="dashed")
    ...
    if i == 0:
        ax1.set_ylabel(f"{feature_name_plot} ", fontsize=15)
    ax1.set_xlabel("Movement number", fontsize=12)
    ax1.xaxis.set_tick_params(labelsize=12)
    ax1.yaxis.set_tick_params(labelsize=12)
    ax1.spines[['right', 'top']].set_visible(False)
    ax1.set_title(f"{channel}", fontsize=12)
    ...
    for block in range(2):
        ...
        not_nan_idx = ~np.isnan(feature_av).any(axis=1)
        # Loop over conditions
        for j in range(2):
            bp = ax2.boxplot(x=feature_av[not_nan_idx, j],
                        positions=[bar_pos[j]],
                        widths=box_width,
                        patch_artist=True,
        ...
        for dat in feature_av[not_nan_idx, :]:
        ...
        # Add statistics
        z, p = scipy.stats.wilcoxon(x=feature_av[not_nan_idx, 0], y=feature_av[not_nan_idx, 1])
        ...
        ax2.text(block-box_width, np.nanpercentile(feature_av, 99)+5, f"p = {np.round(p, 3)}", weight=sig, fontsize=11)
        #ax2.text(block-box_width, 63, f"p = {np.round(p, 3)}", weight=sig, fontsize=13)

        # Add legend
        if i == len(channels) - 1:
            ax2.legend([bps[0]["boxes"][0], bps[1]["boxes"][0]], ['Slow', 'Fast'],
                       loc='lower center', bbox_to_anchor=(0.95, 0.5),
                       prop={'size': 13})

    # Adjust subplot
    if method == "normalized": ax2.axhline(0, linewidth=1, color="black", linestyle="dashed")
    ax2.set_xticks(ticks=[0, 1], labels=["Stimulation", "Recovery"], fontsize=11)
    ax2.set_yticks([])
    ax2.spines[['right', 'top']].set_visible(False)
    u.despine()

# Adjust plot
plt.subplots_adjust(bottom=0.1, left=0.1, top=0.9, wspace=0.3, hspace=0.3)
plt.suptitle(f"{band[0]}-{band[1]} Hz", fontsize=16, y=0.95)

# Save
plot_name = os.path.basename(__file__).split(".")[0]
dir_name = os.path.dirname(os.path.realpath(__file__)).split("\\")[-1]
#plt.savefig(f"../../../Figures/{dir_name}/{plot_name}_{feature_name}_{method}_{med}_{n_norm}_{n_cutoff}.svg", format="svg", bbox_inches="tight", transparent=True)
#plt.savefig(f"../../../Figures/{dir_name}/{plot_name}_{feature_name}_{method}_{med}_{n_norm}_{n_cutoff}.png", format="png", bbox_inches="tight", transparent=True)
