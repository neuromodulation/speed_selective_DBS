...
for i, sig_window in enumerate([sig_window1, sig_window2]):
    ...
    plot_name = os.path.basename(__file__).split(".")[0]
    dir_name = os.path.dirname(os.path.realpath(__file__)).split("\\")[-1]
    plt.savefig(f"../../../Figures/{dir_name}/{plot_name}_{np.round(sig_window[0], 2)}-{np.round(sig_window[1], 2)} sec.pdf", format="pdf", bbox_inches="tight",
                transparent=True)
    plt.savefig(f"../../../Figures/{dir_name}/{plot_name}_{np.round(sig_window[0], 2)}-{np.round(sig_window[1], 2)} sec.png", format="png", bbox_inches="tight",
                transparent=False)
plt.show()