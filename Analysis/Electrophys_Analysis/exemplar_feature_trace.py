...
# Set channels
ch_names = ["ECOG_R_2_CAR", "SPEED_MEAN", "peaks"]
...
tmin = -int(0.7 * samp_freq)
tmax = int(0.7 * samp_freq)
data_all = []
speed_all = []
for idx in peaks_idx:
...
feature_all = np.array(data_all)
speed_all = np.array(speed_all)

for i, (feature, speed) in enumerate(zip(data_all, speed_all)):
    ...
    fig, (ax1, ax2) = plt.subplots(2, 1, height_ratios=[1, 5], figsize=(3, 1.7), constrained_layout=True)
    ...
    im = ax2.imshow(feature, aspect="auto")
    ...
    plt.savefig(
        f"../../../Figures/{dir_name}/features_move/{plot_name}_{i}.pdf",
        format="pdf", bbox_inches="tight", transparent=True)
    plt.savefig(
        f"../../../Figures/{dir_name}/features_move/{plot_name}_{i}.png",
