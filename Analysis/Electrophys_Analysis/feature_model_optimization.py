...
def run_nested_cross_val(setting):
    ...
    filename = f"feature_model_optimization_{setting}.xlsx"
    ...
    for count, n in enumerate(range(n_outer)):
        ...
        def objective_function(samp_freq, seg_ms, n_stack, learning_rate, depth):
            ...
            stream.run(data=data, out_dir="", experiment_name=f"optimization_{setting}")
            ...
            try:
                ...
                """y_test = feature_reader.decoder.all_ch_results["y_test"][0]
                y_test_pr = feature_reader.decoder.all_ch_results["y_test_pr"][0]
                fig, ax = plt.subplots()  # figsize=(1.3, 1.3))
                ax.plot(np.array(y_test), color="black", label="speed")
                ax.plot(y_test_pr, color="red", label="predicted speed")
                ax.legend()
                plt.show()"""
...
if __name__ == "__main__":
    ...
    settings_decoding = ["ECoG_combined", "ECoG_LFP_combined"] + ecog_names + lfp_names
