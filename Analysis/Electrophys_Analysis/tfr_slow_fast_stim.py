...
# Get epochs which were stimulated (slow/fast)
stim = np.any(epochs.get_data(["STIMULATION"])[:,:, ((epochs.times < 0.5) & (epochs.times > -0.5))], axis=-1).squeeze()
slow_block_idx = epochs.get_data(["STIM_CONDITION"])[:,:, epochs.times == 0].squeeze() == 0
fast_block_idx = epochs.get_data(["STIM_CONDITION"])[:,:, epochs.times == 1].squeeze() == 1
...
for i, freq in enumerate(frequencies):
    ...
    for j, time in enumerate(tfr.times):
        t, p = stats.ttest_ind(a=tfr_data[slow_stim_idx, i, j], b=tfr_data[fast_stim_idx, i, j])
        res[0, i, j] = t
        res[1, i, j] = p
...
tfr_slow = tfr[slow_stim_idx].average().apply_baseline(baseline=(None,None), mode="percent")
tfr_fast = tfr[fast_stim_idx].average().apply_baseline(baseline=(None,None), mode="percent")
tfr_diff = tfr_slow.copy()
tfr_diff.data = tfr_fast.data - tfr_slow.data
tfr_diff.plot(axes=axes[0], colorbar=True, cmap=cmap, show=False, tmax=-0.18, tmin=-0.5)
...
for i, idx in enumerate([slow_stim_idx, fast_stim_idx]):
