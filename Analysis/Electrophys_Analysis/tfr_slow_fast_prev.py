...
#raw._data[ch_names.index(target_chan_name), :] = raw.get_data(target_chan_name) - raw.get_data(["ECOG_R_01_SMC_AT", "ECOG_R_02_SMC_AT", "ECOG_R_03_SMC_AT"]).mean(axis=0)
#raw._data[ch_names.index(target_chan_name), :] =  raw.get_data(["LFP_R_02_STN_MT", "LFP_R_03_STN_MT", "LFP_R_04_STN_MT"]).mean(axis=0)
...
for i, ps in enumerate(peak_speed):
    if i > 1 and np.all(ps < peak_speed[i-2:i]):
        ...
        fast_idx.append(i)
fast_idx = np.array(fast_idx)
slow_idx = np.array(slow_idx)
...
# Smooth tfr
tfr.data = uniform_filter1d(tfr.data, size=int(sfreq*0.15), axis=-1)

# Decimate tfr
tfr.decimate(decim=40)

# Normalize to % by using the previous 2 epochs (just as for the movement speed classification)
tfr_data = tfr.data.copy()
tfr_data_new = tfr_data.copy()
for i, tfr_epoch in enumerate(tfr_data):
    if i > 2:
        previous_mean = np.mean(tfr_data[i-2:i, :, :, :], axis=0)
        tfr_data_new[i, :, :, :] = 100 * ((tfr_epoch - previous_mean) / previous_mean)
tfr.data = tfr_data_new
slow_idx = slow_idx[slow_idx > 2]
fast_idx = fast_idx[fast_idx > 2]

# Plot the tfr for slow and fast movements
# Plot
fig, axes = plt.subplots(nrows=1, ncols=2, figsize=(16.8, 6))
# Plot change in power
tfr[slow_idx].average().plot(axes=axes[0], colorbar=True, cmap=cmap, show=False)
tfr[fast_idx].average().plot(axes=axes[1], colorbar=True, cmap=cmap, show=False)
#plt.show()

# Compute difference between tfr of slow and fast movements
tfr_data = tfr.data.copy().squeeze()
...
tfr_slow = tfr[slow_idx].average()
tfr_fast = tfr[fast_idx].average()
