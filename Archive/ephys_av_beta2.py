...
# Delete epochs that are too close to the stimulation
"""sample_stim = events[np.where(events[:, 2] == 10004)[0], 0]
events_no_stim = []
for row in events:
    if not np.any(np.abs((sample_stim - row[0])) < 500):
        events_no_stim.append(row)
events_no_stim = np.array(events_no_stim)"""
...
u.add_new_channel(raw, new_chan, "bipolar_4_5")
new_chan = np.diff(raw.get_data(["ECOG_R_05_SMC_AT", "ECOG_R_06_SMC_AT"]), axis=0)
u.add_new_channel(raw, new_chan, "bipolar_5_6")
new_chan = np.diff(raw.get_data(["ECOG_R_03_SMC_AT", "ECOG_R_04_SMC_AT"]), axis=0)
u.add_new_channel(raw, new_chan, "bipolar_3_4")
new_chan = np.diff(raw.get_data(["ECOG_R_02_SMC_AT", "ECOG_R_03_SMC_AT"]), axis=0)
u.add_new_channel(raw, new_chan, "bipolar_2_3")
new_chan = np.diff(raw.get_data(["ECOG_R_01_SMC_AT", "ECOG_R_02_SMC_AT"]), axis=0)
u.add_new_channel(raw, new_chan, "bipolar_1_2")

# Extract epochs centered on peak speed (omit trials with stimulation)
trials = raw.get_data(["TRIAL"])
blocks = raw.get_data(["BLOCK"])
peak_idx = []
for i_block in range(1, int(np.max(blocks)) + 1):
    for i_trial in range(1, int(np.max(trials)) + 1):
        mask = np.where(np.logical_and(blocks == i_block, trials == i_trial))[1]
        try:
            if not np.any(raw.get_data(["STIMULATION"])[:, mask]):
                peak_idx.append(mask[np.argmax(raw.get_data(["SPEED_MEAN"])[:, mask])])
        except:
            pass
n_moves = len(peak_idx)
events = np.stack((peak_idx, np.zeros(n_moves), np.ones(n_moves))).T
epochs = mne.Epochs(raw, events=events.astype(int), event_id=1, tmin=-0.5, tmax=0.5)


# Plot the average power spectrum of the epochs
ecog_bipolar = raw.info["ch_names"][-5:]
epochs.plot_psd(fmin=3, fmax=140, average=False, picks=ecog_bipolar)

# Plot the power over time
fig, ax = plt.subplots(nrows=5, ncols=1)
for i, channel in enumerate(ecog_bipolar):
    frequencies = np.arange(5, 35, 1)
    power = mne.time_frequency.tfr_morlet(epochs, n_cycles=2, return_itc=False,
                                          freqs=frequencies, decim=3)
    power.plot([channel], axes=ax[i], show=False, baseline=(-0.1, 0.1))
plt.show()
